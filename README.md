...
Plan files may exceed the AWS Lambda 6 MB payload/response limit, so `ryhti_client`
...
- **Import** (`import_plan`): first call the `get_upload_url` action, which returns
  ...
  (instead of the former inline `data.plan_json`).

Uploaded and exported files expire from the bucket automatically after one day.
In the AWS environment the bucket is created by terraform (`infra/s3.tf`) and passed
to the lambda in the `RYHTI_FILES_BUCKET` environment variable. In local development
a [MinIO](https://min.io/) container in `docker-compose.dev.yml` stands in for S3;
presigned URLs generated by the local lambda point at `http://minio:9000/...`, which
resolves inside the compose network — when calling them from the host, replace the
host with `localhost:9000` but keep sending `Host: minio:9000` as a header so the
